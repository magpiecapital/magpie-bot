...
### 3a. ✅ Both programs BUILD to deployable `.so` (verified 2026-08-14)

The Sec3-hardened code is not just `cargo check`-clean — it compiles to deployable SBF artifacts.
Confirmed on the dev box against the current Solana SBF toolchain:

| Program | Artifact | Size | sha256 (this build) |
|---|---|---|---|
| V1 `magpie-lending` | `target/deploy/magpie_lending.so` | 465,328 B | `2621723ba90a9d457707da76dafd3359e1e949ef1441f6325db5fbdbe5267d8b` |
| V3 `magpie-lending-v3` | `target/deploy/magpie_lending_v3.so` | 500,848 B | `763d698c442cf6c20a5a6bea4731c85e608ccfd80e392d26e16d8587cd988300` |

**Toolchain:** `solana-cargo-build-sbf 2.1.0`, `platform-tools v1.43`, `rustc 1.79.0`.

> ⚠️ **Hashes are build provenance for THIS machine, not a byte-match contract.** With
> `lto = "fat"` + `codegen-units = 1`, the `.so` can embed absolute build paths, so a rebuild
> elsewhere may hash differently while being logically identical. If you need a byte-reproducible
> artifact, build with `anchor build --verifiable` (docker) in the deploy env. The security-relevant
> checks are the source (PR #674) + the §4 devnet lifecycle, not a hash equality.

### 3b. ⛔ SBF TOOLCHAIN RECONCILIATION — required or the build fails before it starts

The repo's `Cargo.lock` (generated by a modern host cargo, 1.97) resolves several transitive crates
to versions that need **Cargo `edition2024` / rustc ≥1.85**, which the SBF toolchain's **Cargo 1.79
cannot parse** — the build dies with `feature 'edition2024' is required` / `requires rustc 1.85.0`
before compiling a single line. `.so` production requires pinning those transitive deps **down** to
the last SBF-1.79-compatible versions. **These are pure lockfile downgrades of utility crates — zero
program-logic change.** Both `Cargo.lock`s are `.gitignore`d, so this recipe (not a committed lock)
is the reproducible source of truth. In each program dir:

```
cargo update -p proc-macro-crate@<cur> --precise 3.2.0   # cascades toml_edit 0.25→0.22 (drops edition2024)
cargo update -p indexmap            --precise 2.7.1       # 2.14 needs edition2024
cargo update -p blake3              --precise 1.5.5       # cascades digest 0.11→0.10, crypto-common 0.2→0.1
cargo update -p unicode-segmentation --precise 1.12.0     # 1.13 needs rustc 1.85
cargo update -p zeroize_derive      --precise 1.4.2       # 1.5 needs edition2024
cargo update -p zeroize             --precise 1.8.1       # 1.9 needs edition2024 (skip if a dep already caps <1.4)
```

Then `cargo build-sbf`. If a *new* edition2024/MSRV crate appears after a dep bump, the error names it
and the exact `cargo update --precise <older>` to run — repeat until `Finished`. **V1 build note:** it
uses `anchor-lang = { workspace = true }`, so it must build as a workspace member — temporarily add
`programs/magpie-lending` to the root `Cargo.toml` `members`, build via
`--manifest-path programs/magpie-lending/Cargo.toml`, then **revert the root `Cargo.toml` + `Cargo.lock`**
(don't drift the shared workspace lock). V3 builds standalone.
> Permanent fix (optional): `solana-install update` to a platform-tools whose Cargo supports
> edition2024, and this whole pinning step disappears.
