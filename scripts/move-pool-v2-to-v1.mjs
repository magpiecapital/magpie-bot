#!/usr/bin/env node
/**
 * One-shot: move 30 SOL of V2 pool liquidity into V1.
 *
 * Operator-authorized 2026-06-14. V2 has low usage; V1 needs more.
 *
 ...
 *
 *   3. Deposit the full 30 SOL into V1 in a single tx. V1's deposit
 ...
 *
 * Usage:
 *   railway run --service magpie-bot -- node scripts/move-pool-v2-to-v1.mjs [--dry-run] [--amount-sol 30] [--chunk-sol 0.1]
 */
import {
  Connection,
  Keypair,
...
function parseArgs() {
  ...
  const out = { dryRun: false, amountSol: 30, chunkSol: 0.1 };
...
async function main() {
  ...
  if (v1Pool.toBase58() !== STATED_V1 || v2Pool.toBase58() !== STATED_V2) {
  ...
  // ── Step 3: unwrap + deposit into V1 ──
  console.log(`\n[Step 3] Unwrap wSOL ATA → native SOL, then V1 deposit ${amountSol} SOL…`);
  ...
  // Re-wrap exactly amountLamports + deposit into V1
  ...
  const depositIx = await programV1.methods
  ...
  console.log(`  V1 deposit OK: ${sig3}`);
