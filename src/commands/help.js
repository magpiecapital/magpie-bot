import { InlineKeyboard } from "grammy";
import { isAdmin } from "../services/admin.js";

export async function handleHelp(ctx) {
  const lines = [
    "🏦 *Magpie Help*",
    "",
    "_I'm Pip — your AI agent. Tap any command below, or just talk to me in plain English._",
    "",
    "*Navigation*",
    "/home — main menu",
    "/help — this page",
    "",
    "*Account*",
    "/me — wallet, tier, stats, referral code",
    "/deposit — show your deposit address",
    "/notify — manage notifications + auto-repay",
    "",
    "*Loans*",
    "/supported — accepted collateral with live prices",
    "/simulate <symbol> <amount> [tier] — preview a loan",
    "/borrow — take out a SOL loan",
    "/reborrow — quick re-borrow (same token + tier as last loan)",
    "/positions — active loans + live health (alias: /loans)",
    "/tx <sig> — quick lookup of any Solana tx",
    "/walletlookup <pubkey> — Magpie footprint of any wallet (rep check)",
    "/repay — repay and reclaim collateral",
    "/partialrepay — pay down part of a loan",
    "/topup — add collateral (improves health, no fee)",
    "/extend — extend a loan (tier fee: 3%/2%/1.5%)",
    "/takeprofit <loan_id> at 2x — auto-close + sell when collateral hits target (also: /tp, /sell)",
    "/stoploss <loan_id> at 0.7x — auto-close before downside gets worse (also: /sl)",
    "/trailingstop <loan_id> 10% — stop-loss with a floor that floats UP with each new high (also: /ts)",
    "/bracket <loan_id> tp=2x sl=0.7x — arm BOTH a TP and SL in one shot; first to fire cancels the other",
    "/preview <loan_id> at 2x — dry-run any TP without committing a slot (also: /checkarm)",
    "/previewsl <loan_id> at 0.7x — dry-run any SL without committing (also: /checkarmsl)",
    "/limitorders — show all your armed TP / SL / trailing orders",
    "/modifyorder <order_id> price=… slip=… trailing=… — change a live order without re-arming",
    "/cancellimitorder <order_id> — cancel a specific order",
    "/cancelbracket <loan_id> — cancel both legs of a bracket in one shot",
    "/fixarm — recover any V4 loan whose auto-sell didn't finish arming",
    "/history — last 10 loans",
    "/price <symbol|mint> — oracle price + max loan per tier",
    "",
    "*Wallet*",
    "/wallet — show your deposit address",
    "/deposit — same as /wallet",
    "/import — import an existing Solana wallet (advanced)",
    "/withdraw — send SOL or tokens out",
    "/export — export your private key (⚠️ custodial)",
    "",
    "*Credit & Risk*",
    "/credit — view your 300-850 credit score",
    "/risk <symbol> — AI risk assessment for a token",
    "",
    "*Earn*",
    "/refer — earn 5% of friends' loan fees (lifetime, paid in SOL)",
    "/holders — $MAGPIE holders earn 70% of all fees pro-rata (weekly distribution)",
    "",
    "*Marketplace*",
    "/lend — lending marketplace (create pools, deposit, browse)",
    "",
    "*Tokens*",
    "/submit <mint|symbol> — submit a token for collateral approval",
    "",
    "*Stats*",
    "/stats — protocol-wide stats",
    "/version — bot version + uptime",
    "/whatsnew — recent additions to the protocol",
    "",
    "*Support*",
    "/support — diagnose a loan, check a tx, or message the team",
    "/mytickets — your support history",
    "",
    "*Site*",
    "/site — open magpie.capital (does almost everything this bot does)",
    "/link <code> — link a wallet connected on the site",
    "",
    "*Community*",
    "/community — join the public Magpie group chat (@magpietalk)",
    "  • This bot here is your private *wallet*. Only you see it.",
    "  • The community group is a separate, public chat for discussion.",
    "  • No one will ever DM you first claiming to be Magpie. We don't do DMs.",
    "",
    "*Privacy & security*",
    "/privacy — what data we store + how to control it",
    "/security — single-screen security view + lock buttons",
    "/lock [hours] — emergency pause on site signed actions (24h default)",
    "/lock 0 — clear the lock",
    "/exportdata — DM you a JSON file of your account data",
    "/signedhistory — last 10 signed site actions on your account",
    "/testalert — fire a test security DM at yourself",
    "",
    "*How it works*",
    "1. Deposit memecoins to your Magpie wallet",
    "2. Pick a tier:  30%/2d · 25%/3d · 20%/7d",
    "3. Receive SOL instantly (Express 3%, Quick 2%, Standard 1.5% fee)",
    "4. Repay before due to reclaim your bag",
    "5. Miss the deadline or drop below 1.1x → liquidation",
  ];

  if (isAdmin(ctx.from?.id)) {
    lines.push(
      "",
      "*Admin*",
      "/pause, /resume — toggle new borrows",
      "/adminstatus — system overview",
      "/enablemint <mint> <symbol> <decimals> [name]",
      "/disablemint <symbol|mint>",
      "/broadcast <message> — DM everyone",
      "/reviewtokens — review auto-screened tokens",
      "/siteops — site-action telemetry (24h locks, withdraws, signed actions)",
      "/adminlock <user> [hours] — force-lock a user (DMs them)",
      "/adminunlock <user> — clear a user's lock (DMs them)",
      "/sitedisable [reason] — STOP every signed site endpoint (503)",
      "/siteenable — re-enable signed site endpoints",
      "/sitestate — show current global state",
      "/announce <msg> [expires:hours] — set dashboard banner (info)",
      "/announcewarn / /announcecrit — same, with severity",
      "/announceclear — remove the banner",
    );
  }

  const kb = new InlineKeyboard()
    .text("🏠 Home", "start:home")
    .text("💰 Borrow", "start:borrow")
    .text("📋 Wallet", "fallback:deposit");

  await ctx.reply(lines.join("\n"), { parse_mode: "Markdown", reply_markup: kb });
}
