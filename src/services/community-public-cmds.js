...
export async function handleCommunityFees(ctx) {
  const text = [
    ...
    RULE,
    row("LPs", "80%"),
    row("$MAGPIE holders", "10%"),
    row("Referrers", "5%"),
    row("LP loyalty pool", "2%"),
    row("Protocol", "3%"),
    RULE,
    "```",
    ``,
...
export async function handleCommunityCredit(ctx) {
  const text = [
    ...
    ``,
    ``,
    `*Tier benefits (new 2026-06-07)*`,
    `• 🥉 Bronze (300-499) → 5 SOL / 10 SOL outstanding (after on-time history)`,
    `• 🥈 Silver (500-649) → 5 SOL / 10 SOL outstanding`,
    `• 🥇 *Gold (650+)* → *10 SOL / 20 SOL outstanding* ← upgraded`,
    `• 💎 *Platinum (750+)* → *10 SOL / 20 SOL outstanding* ← highest tier`,
    ``,
    `*How to check yours*`,
