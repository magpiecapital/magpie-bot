...
export async function handleCommunityFees(ctx) {
  const text = [
    ...
    ``,
    `*Defaulted-loan profit policy*`,
    `When a loan defaults, the protocol seizes + sells the collateral, recovers the principal it lent, and routes the NET PROFIT into the next reward distribution — same 70/10/10/10 split as above (referrer slice rolls back into the holder slice if the borrower has no referrer, so holders effectively get 80%). When \$MAGPIE is the collateral, the seized \$MAGPIE is burned instead of sold. Live numbers: /stats and magpie.capital/stats.`,
    ``,
    `*What is NOT a fee*`,
...
export async function handleCommunityHolders(ctx) {
  const text = [
    ...
    `• When a non-$MAGPIE loan defaults, the protocol's profit (sale proceeds minus principal) also feeds the pool — 70% to holders, 80% if the borrower had no referrer`,
